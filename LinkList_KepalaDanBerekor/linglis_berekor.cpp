...
typePtr awal, akhir, kepala, ekor;
...
void sisipList (fruit IB) {
	...
	bantu = (typeNode *) malloc(sizeof(typeNode));
...
void hapusNode (fruit IH) {
	...
	bantu = kepala;
	while (IH.nama != bantu->next->infoBuah.nama && bantu->next != ekor) {
		bantu = bantu->next;
	}
	if (IH.nama == bantu->next->infoBuah.nama) {
		hapus = bantu->next;
		bantu->next = hapus->next;
		free(hapus);
		cout<<"Data "<<IH.nama<<" berhasil dihapus";
	}
	...
	switch (pilih) {
		case 1 :
			...
			for (int i = 0; i < banyakData; i++) {
				fruit IB;
				typeNode* bantu = (typeNode *) malloc(sizeof(typeNode));
				do {
					bantu = kepala;
					error = false;
				cout << "ID buah    : ";
		...
		case 2 : 
			...
			cout << "----------------------------------------------" << endl;
			if (cekList()) {
				cout << "Hmm nice try! List masih kosong nich, diisi yaa :D" << endl << endl;
			} else {
				tampilindariDepan();				
			}
			break;
		case 3 :
			...
			if (cekList()) {
				...
				fruit IH;
				cout << "ID buah yang ingin dihapus = ";
				cin >> IH.nama;
				hapusNode(IH);
			}
			break;
		default :
			...
			cout << "                  DATA NAMA BUAH              " << endl;
			cout << "----------------------------------------------" << endl; 
			cout << "Hmm nice try! Menu tidak tersedia :D" << endl << endl;
	}
	do {
